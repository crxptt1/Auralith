...
import {Plus,Upload,FileDown,Trash2,ArrowRight,AlignLeft,Copy,Check,BookOpen,Info} from 'lucide-react';
...
export function ScriptLab({project,update,importText,exportText,goVoices}:{project:Project;update:(fn:(p:Project)=>Project)=>void;importText:()=>void;exportText:()=>void;goVoices:()=>void}){
 ...
 <div className="script-layout"><section className="script-paper"><div className="script-toolbar"><div className="segmented">{(['all','A','B','C'] as const).map(r=><button key={r} className={filter===r?'active':''} onClick={()=>setFilter(r)}>{r==='all'?t("Wszystkie"):t("Warstwa {0}", [r])}</button>)}</div><div className="inline-actions"><button className="icon-button" title={t("Importuj tekst")} aria-label={t("Importuj tekst")} onClick={importText}><Upload size={17}/></button><ImportHelp/><button className="icon-button" title={t("Eksportuj tekst")} aria-label={t("Eksportuj tekst")} onClick={exportText}><FileDown size={17}/></button><button className="icon-button" title={t("Kopiuj tekst")} aria-label={t("Kopiuj tekst")} onClick={async()=>{await navigator.clipboard.writeText(project.script.map(l=>`${l.role}: ${l.text}`).join('\n'));setCopied(true);setTimeout(()=>setCopied(false),1600);}}>{copied?<Check size={17}/>:<Copy size={17}/>}</button></div></div>
