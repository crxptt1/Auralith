...
export function Comparison({project,update}:{project:Project;update:(fn:(p:Project)=>Project)=>void}){
 ...
 const [order,setOrder]=useState<[string,string]|null>(null);const [votes,setVotes]=useState<string[]>([]);const [heard,setHeard]=useState<string[]>([]);const [playing,setPlaying]=useState<string|null>(null);const [error,setError]=useState('');const [volume,setVolume]=useState(.2);
 ...
 async function listen(id:string){const el=player.current;const r=eligible.find(x=>x.id===id);if(!el||!r||!a||!b)return;el.pause();const target=Math.min(a.metrics.integratedLufs!,b.metrics.integratedLufs!);el.volume=playbackGain(volume)*Math.pow(10,(target-r.metrics.integratedLufs!)/20);el.src=r.url;try{await el.play();setPlaying(id);setHeard(prev=>[...new Set([...prev,id])]);}catch(error){if(error instanceof DOMException&&error.name==='AbortError')return;setError(t("Nie można odtworzyć tego pliku. Wybierz dostępny eksport."));}}
