...
export const normalizeLocale = (value) => value === 'pl' ? 'pl' : 'en';
let locale = 'en';
try {
    locale = normalizeLocale(globalThis.localStorage?.getItem(localeStorageKey));
}
catch { /* Storage may be unavailable in a sandbox. */ }
const listeners = new Set();
export const getLocale = () => locale;
export const subscribeLocale = (listener) => { listeners.add(listener); return () => { listeners.delete(listener); }; };
export function setLocale(value) { const next = normalizeLocale(value); try {
    globalThis.localStorage?.setItem(localeStorageKey, next);
}
catch { } if (locale === next)
    return; locale = next; for (const listener of listeners)
    listener(); }
export function t(key, values = [], language = locale) { const template = language === 'en' ? (english[key] ?? key) : key; return template.replace(/\{(\d+)\}/g, (token, index) => index in values ? String(values[Number(index)]) : token); }
...
function messageEntries(language) { return language === 'en' ? translations : entries.map(([pl, en]) => [en, pl]); }
...
export function tMessage(message) {
    ...
    for (const [source, target] of messageEntries(locale)) {
        ...
        if (match)
            return target.replace(/\{(\d+)\}/g, (token, i) => match[indices.indexOf(Number(i)) + 1] ?? token);
    }
    let result = message;
    for (const [source, target] of fragments[locale])
