...
async function render(request){
 ...
 const controller=new AbortController();renderJob=controller;
  try{const result=await engine.render({...p,duration,layers:p.layers.map(l=>({...l,offset:Math.min(l.offset,duration)})),format:request.format,outputPath,preview,autoExtend:!preview},{signal:controller.signal,onProgress:sendProgress});const completed={...result,id,url:mediaUrl(result.path)};if(preview){const {removePreviousPreviews}=require('./preview-cleanup.cjs');await removePreviousPreviews({renders:store.renders,keepPath:result.path});}else await atomicWrite(outputPath+'.json',JSON.stringify({appVersion:app.getVersion(),createdAt:new Date().toISOString(),project:{...p,duration:result.duration,exports:[],comparisons:[]},result:completed},null,2));return completed;}
...
async function register(){
 ...
 handle('updates:check',()=>updates.check());
  handle('updates:open-release',()=>{const releasePage=updates.releasePage();if(!releasePage)throw new Error('Update source is unavailable.');return shell.openExternal(releasePage);});
