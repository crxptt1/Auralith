...
async function trashRenderedExport({renders,project,exportId,projects,trashItem}){
 const {file,existing}=await ownedFiles({renders,project,exportId,projects});
...
async function ownedFiles({renders,project,exportId,projects}){
 ...
 if(!same(path.dirname(file),renders)||!/^\.(wav|mp3|flac)$/i.test(path.extname(file)))throw new Error('Only application-owned render files can be removed.');
 for(const p of projects){if((p.layers||[]).some(l=>same(l.path,file))||(p.background?.kind==='file'&&same(p.background.path,file))||(p.exports||[]).some(r=>same(r.path,file)&&!(p.id===project.id&&r.id===exportId)))throw new Error('This audio is used by another project or export.');}
 const existing=[];
...
async function deleteRenderedExport(options){const {existing}=await ownedFiles(options);for(const file of existing)await fs.rm(file,{force:true});}
module.exports={trashRenderedExport,deleteRenderedExport};
